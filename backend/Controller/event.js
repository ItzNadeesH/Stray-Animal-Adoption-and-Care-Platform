...
const createEvent = async (req, res) => {
   try {
      upload(req, res, async (err) => {
         const { name, description, date, start_time, end_time, location, user, fundState, state } = req.body;
         if (name == '' || description == '' || date == '', start_time == '', end_time == '', location == '' || user == '' || fundState == '' || state == '') {
...
const updateEvent = async (req, res) => {
   try {
      upload(req, res, async (err) => {
         const { id } = req.params;
         const { name, description, date, start_time, end_time, location, user, state } = req.body;

         if (name == '' || description == '' || date == '', start_time == '', end_time == '', location == '' || user == '' || state == '') {
            const filePath = req.files.image[0].path;
            fs.unlinkSync(filePath);
            return res.status(400).json({ error: true, message: "Please fill all fields" });
         }

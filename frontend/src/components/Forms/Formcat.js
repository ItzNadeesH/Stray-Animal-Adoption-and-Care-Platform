...
function Formcat() {
  ...
  const fetchForms = async () => {
    try {
      const response = await axios.get('/Form/get');
  ...
  const handleDelete = async (formId) => {
    try {
      await axios.delete(`/Form/delete/${formId}`);
    ...
    <div className="max-w-xl mx-auto border-2 border-gray-300 rounded-lg shadow p-4">
      ...
      <div className="flex flex-wrap gap-5 justify-center">
        {filteredForms.map((form) => (
          <div
            key={form._id}
            className="p-4 border-2 border-gray-300 rounded shadow-md cursor-pointer w-full sm:w-3/3"
          >
            <p className="mb-2">
            ...
            <p className="mb-2">
              <strong>Number of Fields:</strong> {form.formFields.length}
            </p>
            <div className="flex justify-between">
              <button
                onClick={() => handleUpdate(form._id)}
                className="bg-blue-500 hover:bg-blue-700 text-white font-bold py- px-10 rounded focus:outline-none focus:shadow-outline"
              >
                Edit
              </button>
              <button

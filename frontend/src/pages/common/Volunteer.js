...
function Volunteer() {
   ...
   const handleViewRequest = (volunteerId) => {
      setSelectedVolunteer(volunteerData.find((item) => item._id === volunteerId));
      setIsModalOpen(true);
   }
            ...
            {filteredData.map((item) => (
               <div key={item._id} className="border p-4 shadow-[2px_2px_2px_1px_#0004] hover:shadow-[2px_2px_2px_1px_#0006] rounded-lg hover:cursor-pointer bg-white" >
                  ...
                  <div className='grid grid-cols-1 mt-4'>
                     {(user && user?.role === 'USER') ? (
                        <button className="bg-green-500 hover:bg-green-700 text-white text-sm font-bold py-2 px-4 rounded w-full mr-2" onClick={() => handleViewRequest(item?._id)} >Attend Request</button>
                     ) : (

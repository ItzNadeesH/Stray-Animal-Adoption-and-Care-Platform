...
function AnimalManage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    doc.text(topText16, doc.internal.pageSize.getWidth() / 2, 15, {
      align: "center",
    });

    doc.setFontSize(12);
    const topText14 = "Report Generated: " + new Date().toLocaleString();
    doc.text(topText14, doc.internal.pageSize.getWidth() / 2, 20, {
      align: "center",
    });
  ...
  const handleSearch = async () => {
    ...
    if (filterVaccination !== "") {
      filteredData = filteredData.filter(
        (animal) => animal.vaccinationFrequency === filterVaccination
      );
    }
    ...
    if (filterSearch !== "") {
      filteredData = filteredData.filter(
        (animal) =>
          animal._id.includes(filterSearch) ||
    ...
    <Layout>
      <div className="p-10 ">
        ...
        <div className="flex space-x-3">
          <button
            onClick={() => navigate("/admin/animal/add")}
            className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded my-3 flex items-center"
          >
            <HiPlus className="mr-3" /> Add Animal
          </button>
          <button
            onClick={handleDownloadPDF}
            className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded my-3 flex items-center"
          >
            <HiPlus className="mr-3" /> Download PDF
          </button>
        </div>
        <div className="flex justify-between space-x-10 bg-slate-100 p-3 mb-3 border border-slate-400">
          <div>
            <label htmlFor="filterType" className="text-sm font-medium mr-3">
              Filter by Type:
            </label>
            <select
              id="filterType"
          ...
          <div>
            <label
              htmlFor="filterVaccination"
              className="text-sm font-medium mr-3"
            >
              Filter by Vaccination Period:
            </label>
            <select
              id="filterVaccination"
              className="border border-gray-300 rounded-md px-2 py-1"
              onChange={() => handleSearch()}
            >
              <option value="">All</option>
            ...
            {filteredData.map((item) => (
              <tr key={item._id} className="bg-white">
                ...
                <td className="border px-4 py-1 text-sm">
                  {item.adoption.length}
                </td>
                <td className="border px-4 py-1 text-sm">
                  {item.vaccinationFrequency}
                </td>
        ...
        >
          <div className="bg-sky-200 p-4 rounded-lg shadow-sm shadow-stone-900">
            ...
            <div className="flex">
              <div className="w-[200px] text-sm font-semibold leading-4">
            ...
            <div className="flex justify-end mt-5">
              <button
                className="bg-sky-500 hover:bg-sky-800 px-5 py-2 rounded-lg text-white font-bold"

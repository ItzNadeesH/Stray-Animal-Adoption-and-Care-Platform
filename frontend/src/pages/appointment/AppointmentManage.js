               ...
               <tbody>
                  {filteredData.map((item) => (
                     <tr key={item._id} className="bg-white">
                        ...
                        <td className="border px-4 py-1 text-sm">{item.animalData.name}</td>
                        <td className="border px-4 py-1 text-sm">{new Date(item.requestedDate).toISOString().slice(0, 10).replace('T', ' ')}</td>

            ...
            <tbody>
               {filteredData.map((event) => (
                  <tr key={event._id} className="bg-white">
                     ...
                     <td className="border px-4 py-1 text-sm ">{event.fundState}  {event.fundState === 'NONE' || event.fundState === 'REJECTED' ? (<HiCash onClick={() => handleFundRequest(event._id)} size={20} className="hover:cursor-pointer" />) : <></>}   </td>

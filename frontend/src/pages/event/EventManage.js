...
import { HiTrash, HiPencilAlt, HiPlus, HiCash } from "react-icons/hi";
...
function EventManage() {
   ...
   const [fundRequestEvent, setFundRequestEvent] = useState([]);
   ...
   const handleFundRequest = (id) => {
      fetch(APP_URL + `/api/event/${id}`)
         .then((res) => res.json())
         .then((data) => {
            setFundAmount(0);
            setFundRequestEvent(data);
      ...
      if (search) {
         filteredData = filteredData.filter((item) => {
            return item.name.toLowerCase().includes(search.toLowerCase()) ||
               ...
               item.start_time.toLowerCase().includes(search.toLowerCase()) ||
               item.end_time.toLowerCase().includes(search.toLowerCase()) ||
               item.location.toLowerCase().includes(search.toLowerCase()) ||
               item.attendees.toString().includes(search.toLowerCase()) ||
   ...
   const handleFundRequestSubmit = async () => {
      const event = fundRequestEvent._id;
      if (fundAmount <= 0) {
            ...
            <table className="w-full">
               <thead>
                  <tr>
                     ...
                     <th className="border px-4 py-1 bg-slate-400">Start Time</th>
                     <th className="border px-4 py-1 bg-slate-400">End Time</th>
               ...
               <tbody>
                  {filteredData.map((event) => (
                     <tr key={event._id} className="bg-white">
                        ...
                        <td className="border px-4 py-1 text-sm">{event.start_time}</td>
                        <td className="border px-4 py-1 text-sm">{event.end_time}</td>
                        ...
                        <td className="border px-4 py-1 text-sm"><div className="flex justify-between">{event.fundState}  {event.fundState === 'NONE' || event.fundState === 'REJECTED' ? (<HiCash onClick={() => handleFundRequest(event._id)} size={20} className="hover:cursor-pointer" />) : <></>}  </div> </td>
                        ...
                        <td className="border px-4 py-1 ">
                           <div className="flex justify-center">
                              ...
                              <HiTrash onClick={() => handleDelete(event._id)} size={20} className="mr-4 hover:cursor-pointer" />
                           </div>
                        </td>
                     </tr>
                  ))}
            ...
            <Modal isOpen={isFundRequestModalOpen} onRequestClose={() => setIsFundRequestModalOpen(false)} className="modal lg empty" overlayClassName="modal-overlay" appElement={document.getElementById('root')}>
               <div className=" bg-sky-200 p-3 rounded-lg shadow-[2px_2px_2px_1px_#0004]">
                  ...
                  <div className="flex items-center mb-4">
                     <div className="w-[200px] text-sm font-semibold leading-4">Event Name</div>
                     <div className="w-[10px] px-5">:</div>
                     <input type="text" className="w-full p-2 text-sm" value={fundRequestEvent.name} disabled />
                  </div>
                  <div className="flex items-center mb-4">
                     <div className="w-[200px] text-sm font-semibold leading-4">Event Date</div>
                     <div className="w-[10px] px-5">:</div>
                     <input type="text" className="w-full p-2 text-sm" value={fundRequestEvent.date} disabled />
                  </div>
                  <div className="flex items-center mb-4">

...
import { HiTrash, HiPencilAlt, HiPlus, HiDocument } from "react-icons/hi";
...
function VolunteerManage() {
   ...
   const handleSeparateRequest = async (id) => {

      const request = volunteerData.find((item) => item._id === id);
      const response = await fetch(APP_URL + `/api/volunteerRespond/request/${id}`);
      ...
      headers1.forEach((header) => {
         row.push(request[header.dataKey]);
      });
      data.push(row);
      ...
      const headers = [
         ...
         { title: "User Email", dataKey: "email" },
         { title: "Signature", },
      ];

      data = [];
      const respondedVolunteers = await response.json();
      respondedVolunteers.forEach((event) => {
         const row = [];
         headers.forEach((header) => {
            if (header.dataKey === "no") {
               row.push(respondedVolunteers.indexOf(event) + 1);
            } else {
               row.push(event[header.dataKey]);
            }
         });
      ...
      doc.setFontSize(16)
      const topText16 = "Stray Animal Adoption and Care Platform - Volunteer Attendance Report";
      doc.text(topText16, doc.internal.pageSize.getWidth() / 2, 15, { align: 'center' });
   ...
   const handleSearch = async () => {
      ...
      if (search) {
         filteredData = filteredData.filter((item) => {
            return item.skill.toLowerCase().includes(search.toLowerCase()) ||
               item.description.toLowerCase().includes(search.toLowerCase()) ||
               item.district.toLowerCase().includes(search.toLowerCase()) ||
               item.state.toLowerCase().includes(search.toLowerCase()) ||
               item.onDate.toLowerCase().includes(search.toLowerCase()) ||
            ...
            <table className="w-full">
               <thead>
                  <tr>
                     ...
                     <th className="border px-4 py-1 bg-slate-400">District</th>
                     <th className="border px-4 py-1 bg-slate-400">Volunteer Count</th>
               ...
               <tbody>
                  {filteredData.map((item) => (
                     <tr key={item._id} className="bg-white">
                        ...
                        <td className="border px-4 py-1 text-sm">{item.maxVolunteers} / {item.volunteers ?? 0}</td>
                        <td className="border px-4 py-1 text-sm">{new Date(item.onDate).toISOString().slice(0, 10).replace('T', ' ')}</td>
                        ...
                        <td className="border px-4 py-1 text-sm">
                           <div className="flex justify-between">
                              ...
                              <TbListDetails onClick={() => handleViewVolunteers(item._id)} size={20} className="mr-4 hover:cursor-pointer" />
                              <HiDocument onClick={() => handleSeparateRequest(item._id)} size={20} className="hover:cursor-pointer" />
            ...
            <Modal isOpen={isModalOpen} onRequestClose={() => setIsModalOpen(false)} className="modal lg empty" overlayClassName="modal-overlay" appElement={document.getElementById('root')}>
               <div className=" bg-sky-200 p-3 rounded-lg shadow-[2px_2px_2px_1px_#0004]">
                  ...
                  <table className="w-full mb-4">
                     <thead>
                        <tr>
                           <th className="border px-4 py-1 bg-slate-400">No</th>
                           <th className="border px-4 py-1 bg-slate-400">User Name</th>
                           <th className="border px-4 py-1 bg-slate-400">User Email</th>
                        </tr>
                     </thead>
                     <tbody>
                        {respondedVolunteers.map((item, index) => (
                           <tr key={item._id} className="bg-white">
                              <td className="border px-4 py-1 text-sm">{index + 1}</td>
                              <td className="border px-4 py-1 text-sm">{item.username}</td>
